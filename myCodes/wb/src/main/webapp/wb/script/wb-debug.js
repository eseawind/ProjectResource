...
var Wb = {
   ...
   * @param {Boolean} [bottomRight] 是否在右下角显示，默认自顶部中间显示。该参数仅适于用桌面。
   */
  toast: window.Ext && Ext.isTouch ? Ext.toast : function(message, bottomRight) {
    new Ext.ux.window.Notification({
      position: bottomRight ? 'br' : 'tc',
      html: message,
      bodyPadding: 10,
  ...
  /**
   * 在Home主页的信息栏显示通知消息。该方法仅在Home主页内有效，否则使用该方法无任何效果。
   * @param {String} html 显示消息的HTML文本。
   */
  ...
  /**
   * 以对话框的形式显示登录窗口。
   * @param {Function} [beforeFn] 显示登录对话框后的回调函数，适用于touch模式。
   * @param {Function} [afterFn] 登录成功后的回调函数，适用于touch模式。
   */
    ...
    if (Ext.isTouch) {
      ...
      if (touchMask) {
        if (component.maskMsgs.length == 1) {
          touchMask.updateMaskTimeout(Wb.maskTimeout);
          touchMask.show();
        }
  ...
  prompt: function(configs) {
    ...
    win.show();
    if (!configs.win)
      win.setHeight(Math.min(win.getHeight(), Ext.Element.getViewportHeight() - 8));
    Ext.suspendLayouts();
    try {
  ...
  verify: function(components) {
    ...
    function query(item) {
      if (Ext.isTouch) {
        ...
        if (item.getRequired && item.getRequired() && item.getValue && item.isDisabled &&
          notHidden && !item.isDisabled() && Ext.isEmpty(item.getValue())) {
          firstComp = item;
        ...
        if (item instanceof Ext.form.field.ComboBox)
          item.assertValue();
        if (item.validate && !item.hidden && !item.disabled && !item.validate() && !firstComp && !item.inEditor)
          firstComp = item;
      }
      return false;
    }
  ...
  parseBool: function(value, defaultValue) {
    if (Wb.isValue(value) || defaultValue === undefined) {
  ...
  reload: function(component, object, params) {
    if (component instanceof Ext.grid.Panel || component instanceof Ext.data.Store) {
      ...
      options.params = Ext.apply({}, options.params, store.lastOptions.params);
  ...
  getTopColumns: function(grid) {
    var topCols = [],
      columns = Ext.isTouch ? grid.getColumns() : grid.columns;
  ...
  /**
   * 导出表格中的数据为指定格式的输出。
   * @param {GridPanel/Store} 需要导出的表格或关联的store对象。
  ...
  /**
   * 根据table dom中的数据下载对应的Excel文件。
   * @param {DOM} table html表格dom对象。
   * @param {String} [filename] 下载的文件名。
   * @param {String} [title] 标题，缺省将不输出标题。
   */
  getExcel: function(table, filename, title) {
    var i, j, k, l, isDataRow, row, cell, rowData, headers = [],
      rows = [];
    j = table.rows.length;
    for (i = 0; i < j; i++) {
      isDataRow = i > 0;
      ...
      for (k = 0; k < l; k++) {
        cell = row.cells[k];
        if (isDataRow) {
          rowData['f' + k] = cell.textContent || cell.innerText;
        } else {
          headers.push({
            text: cell.textContent || cell.innerText,
            width: cell.clientWidth + 4,
            field: 'f' + k
          });
        }
      }
   ...
   * 并设置窗口isNew属性，用以标识是否为添加或修改操作。如果窗口为实例默认使用单例模式，自动设置closeAction为hide，
  ...
  edit: function(object, configs, isNew) {
    ...
    if (configs.win) {
      ...
      Wb.each(Ext.isTouch ? Wb.getColumns(grid) : grid.columns, function(column) {
  ...
  kvRenderer: window.Ext && Ext.isTouch ? function(value, r) {
    ...
    if (grid instanceof Ext.grid.column.Column)
      grid = grid.up('grid');
    cols = grid.headerCt ? grid.headerCt.getGridColumns() : grid.columns;
  ...
  loadColumns: function(object, reload) {
    ...
    if (Ext.isTouch) {
      ...
      if (reload || Wb.isEmpty(grid.columns)) {
  ...
  invoke: function(dom) {
    var el = Ext.fly(dom);
    Ext.getCmp(Ext.fly(dom).up('div[class~=' + (el.getAttribute('cls') || 'x-grid') + ']').id)
      .appScope[el.getAttribute('method')].apply(dom, [].slice.call(arguments, 1));
  },
  /**
   ...
   */
  setChecked: function(node, checked, ignoreBox) {
    checked = !!checked; //转换为布尔值
    if (!ignoreBox)
      node.data.checked = checked; //仅使用标识即可
    var box, el = Ext.fly(Ext.fly(node.getOwnerTree().view.id + '-record' + node.id));
  ...
  bindChange: function(grid, changeEvent) {
    ...
    if (!grid.changeEventBinded) {
      ...
      Wb.each(grid.columns, function(col) {
        if (col.editor) {
  ...
  print: function(html, title) {
    ...
    var doc = Wb.getDoc(Wb.printFrame);
