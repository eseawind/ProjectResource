package com.shlanbao.tzsc.pms.qm.better.service.impl;
 ...
 */
@Service
public class BetterManageServiceImpl extends BaseService implements BetterManageService{
	@Autowired
	private QmSelfCheckStripDaoI qmSelfCheckStripDao;
	@Override
	public DataGrid queryList(BetterManageQueryBean bean, PageParams pageParams) throws Exception{
		...
		try {
			...
			List<BetterManageQueryBean> list= new ArrayList<BetterManageQueryBean>();
			if(total>0){
				...
				for(QmSelfCheckStrip strip:rows){
					BetterManageQueryBean oneBean = BeanConvertor.copyProperties(strip, BetterManageQueryBean.class);
	...
	public BetterManageBean getBeanData(BetterManageQueryBean bean) throws Exception{
		...
		BetterManageBean queryBean = null;
		String type ="0";
		if(null==list){
		...
		queryBean = new BetterManageBean(lables,type,values1,values2);
