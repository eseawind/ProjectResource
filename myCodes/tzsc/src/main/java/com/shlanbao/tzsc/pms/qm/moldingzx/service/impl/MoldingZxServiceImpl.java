package com.shlanbao.tzsc.pms.qm.moldingzx.service.impl;
 ...
 */
@Service
public class MoldingZxServiceImpl extends BaseService implements MoldingZxService{
	@Autowired
	private QmSelfCheckStripDaoI qmSelfCheckStripDao;
	@Override
	public DataGrid queryList(MoldingZxBean bean, PageParams pageParams) throws Exception{
		...
		try {
			...
			List<MoldingZxBean> list= new ArrayList<MoldingZxBean>();
			if(total>0){
				...
				for(QmSelfCheckStrip strip:rows){
					MoldingZxBean oneBean = BeanConvertor.copyProperties(strip, MoldingZxBean.class);
