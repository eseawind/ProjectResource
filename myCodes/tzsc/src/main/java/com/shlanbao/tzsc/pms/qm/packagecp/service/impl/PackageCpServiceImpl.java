package com.shlanbao.tzsc.pms.qm.packagecp.service.impl;
 ...
 */
@Service
public class PackageCpServiceImpl extends BaseService implements PackageCpService{
	@Autowired
	private QmSelfCheckStripDaoI qmSelfCheckStripDao;
	@Override
	public DataGrid queryList(PackageCpBean bean, PageParams pageParams) throws Exception{
		...
		try {
			...
			List<PackageCpBean> list= new ArrayList<PackageCpBean>();
			if(total>0){
				...
				for(QmSelfCheckStrip strip:rows){
					PackageCpBean oneBean = BeanConvertor.copyProperties(strip, PackageCpBean.class);
