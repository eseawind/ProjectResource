package com.shlanbao.tzsc.wct.qm.self.service.impl;
 ...
 */
@Service
public class QmSelfWctServiceImpl extends BaseService implements QmSelfWctService{
	@Autowired
	private QmSelfCheckStripDaoI qmSelfCheckStripDao;
	@Override
	public DataGrid queryList(QmSelfCheckStripBean bean, int pageIndex) throws Exception{
		...
		try {
			...
			List<QmSelfCheckStripBean> list= new ArrayList<QmSelfCheckStripBean>();
			if(total>0){
				hql.append("order by o.time asc ");
				List<QmSelfCheckStrip> rows = qmSelfCheckStripDao.queryByPage(hql.toString(),pageIndex,10, params);
				for(QmSelfCheckStrip strip:rows){
					QmSelfCheckStripBean oneBean = BeanConvertor.copyProperties(strip, QmSelfCheckStripBean.class);
