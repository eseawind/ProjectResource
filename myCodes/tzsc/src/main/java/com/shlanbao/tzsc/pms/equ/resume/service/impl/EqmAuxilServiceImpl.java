...
import com.shlanbao.tzsc.base.dao.EqmAuxilDaoI;
import com.shlanbao.tzsc.base.mapping.EqmAuxil;
...
@Service
public class EqmAuxilServiceImpl extends BaseService implements EqmAuxilServiceI {
	@Autowired
	private EqmAuxilDaoI eqmAuxilDao;
	
	@Override
	public EqmAuxilBean getById(String id) throws Exception {
		EqmAuxil bean = eqmAuxilDao.findById(EqmAuxil.class, id);
		EqmAuxilBean lastBean = BeanConvertor.copyProperties(bean, EqmAuxilBean.class);
	...
	public void addOrEditBean(String eqmResumeId,EqmAuxilBean[] beans,String userId) throws Exception {
		if(null!=beans){
			for(int i=0;i<beans.length;i++){
				EqmAuxilBean bean = beans[i];
				if(null!=bean.getAuxilId()&&!"".equals(bean.getAuxilId())){//更新
					EqmAuxil updateBean = eqmAuxilDao.findById(EqmAuxil.class, bean.getAuxilId());
					...
					updateBean.setAuxilName(StringUtil.trim(bean.getAuxilName()));
					updateBean.setAuxilNorms(StringUtil.trim(bean.getAuxilNorms()));
					updateBean.setAuxilUnit(StringUtil.trim(bean.getAuxilUnit()));
					updateBean.setAuxilPrice(StringUtil.trim(bean.getAuxilPrice()));
					updateBean.setAuxilNumber(StringUtil.trim(bean.getAuxilNumber()));
					updateBean.setAuxilMoney(StringUtil.trim(bean.getAuxilMoney()));
				}else{//保存
					EqmAuxil eqmBean = BeanConvertor.copyProperties(bean, EqmAuxil.class);
					eqmBean.setAuxilId(UUID.randomUUID().toString());//内部转移主键ID
	...
	public DataGrid queryList(EqmAuxilBean bean,PageParams pageParams) throws Exception {
		...
		hql.append("from EqmAuxil o ");//内部转移
		...
		try {
			...
			if(total>0){*/
				...
				List<EqmAuxil> rows = eqmAuxilDao.query(hql.toString(), params);
				for(EqmAuxil eqmBean:rows){
					EqmAuxilBean oneBean = BeanConvertor.copyProperties(eqmBean, EqmAuxilBean.class);
	...
	public void deleteById(String id) throws Exception {
		...
		if(null!=args&&args.length>0){
			for(int i=0;i<args.length;i++){
				EqmAuxil updateBean = eqmAuxilDao.findById(EqmAuxil.class,args[i]);
