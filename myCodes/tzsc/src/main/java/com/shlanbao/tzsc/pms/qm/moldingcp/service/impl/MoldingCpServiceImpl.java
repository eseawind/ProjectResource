package com.shlanbao.tzsc.pms.qm.moldingcp.service.impl;
 ...
 */
@Service
public class MoldingCpServiceImpl extends BaseService implements MoldingCpService{
	@Autowired
	private QmSelfCheckStripDaoI qmSelfCheckStripDao;
	@Override
	public DataGrid queryList(MoldingCpBean bean, PageParams pageParams) throws Exception{
		...
		try {
			...
			List<MoldingCpBean> list= new ArrayList<MoldingCpBean>();
			if(total>0){
				...
				for(QmSelfCheckStrip strip:rows){
					MoldingCpBean oneBean = BeanConvertor.copyProperties(strip, MoldingCpBean.class);
