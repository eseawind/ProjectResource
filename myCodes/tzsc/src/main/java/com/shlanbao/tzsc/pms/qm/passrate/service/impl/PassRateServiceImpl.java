package com.shlanbao.tzsc.pms.qm.passrate.service.impl;
 ...
 */
@Service
public class PassRateServiceImpl extends BaseService implements PassRateService{
	@Autowired
	private QmSelfCheckStripDaoI qmSelfCheckStripDao;
	@Override
	public DataGrid queryList(PassRateQueryBean bean, PageParams pageParams) throws Exception{
	...
	public PassRateBean getPassRateBean(PassRateQueryBean bean) throws Exception{
		...
		for (int i=0;i<list.size();i++){
			...
			values1[i]=Double.parseDouble(value1);
			
			String value2 = ObjectUtils.toString(arr[3]);
			values2[i]=Double.parseDouble(value2);
		}
		queryBean = new PassRateBean(lables,type,values1,values2);
