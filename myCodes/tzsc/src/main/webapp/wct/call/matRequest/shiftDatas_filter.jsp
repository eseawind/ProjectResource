...
<script type="text/javascript">
	...
	$(function(){
		...
		if(ind==0){
			$("#zl_pz2").attr("disabled",false);
			$("#zl_ss2").attr("disabled",false);
		...
		$.post("${pageContext.request.contextPath}/wct/system/loginWctEqm.do",loginBean,function(json){
			if(json.isSuccess=="true"){
				...
				if(ind==0){
					...
					$("#zl_pz2").attr("disabled",true);
					$("#zl_ss2").attr("disabled",true);
	...
	//产量，设备有效作业率
	function qtyAndZyl(){
		var url="${pageContext.request.contextPath}/wct/eqm/shiftGl/matRequestFilterByQty.do";
		$.post(url,null,function(data){
			...
			for(var i=0;i<zj_sl.length;i++){
				if(i==0){
					$("#zl_pz2").val(zj_sl[0]>0?zj_sl[0]:0);
				}else if(i==1){
					$("#zl_ss2").val(zj_sl[1]>0?zj_sl[1]:0);
				}
			}
			...
			for(var i=0;i<zj_xl.length;i++){
				if(i==0){
					$("#zl_pz1").val(zj_xl[0]>0?zj_xl[0]:0);
				}else if(i==1){
					$("#zl_ss1").val(zj_xl[1]>0?zj_xl[1]:0);
				}
			}
			...
			for(var i=0;i<zj_xt.length;i++){
				if(i==0){
					$("#zl_pz3").val(zj_xt[0]>0?zj_xt[0]:0);
				}else if(i==1){
					$("#zl_ss3").val(zj_xt[1]>0?zj_xt[1]:0);
				}
				if(zj_xt[2]==1){
					//隐藏按钮,确定，仲裁
					$("#buts").hide();
					$("#zl_pz3").attr("disabled",true);
					$("#zl_ss3").attr("disabled",true);
	...
	function submitShiftDatas(type){
	 ...
	 function saveShiftDatas(){
	     ...
	     jConfirm('确定提交吗？', '提示', function(r) {
	    	 if(r){
				  var zl_pzf=$("#zl_pz1").val()+","+$("#zl_pz2").val()+","+$("#zl_pz3").val();
				  var zl_ssf=$("#zl_ss1").val()+","+$("#zl_ss2").val()+","+$("#zl_ss3").val();
				  
				  //发送POST请求
				 $.post("${pageContext.request.contextPath}/wct/eqm/shiftGl/saveShiftFilter.do",{"zl_pzf":zl_pzf,"zl_ssf":zl_ssf},function(data){
					 if(data=='true'){
						 //隐藏按钮,确定，仲裁
					     $("#buts").hide();
						 jAlert("提交成功！","提示");
...
<!-- 成型机start -->
				<div class="roller">
				   ...
				   <br/><br/>
				    <ul class="titleul2">
				      <li>虚领辅料：</li>
				      <li><input type="text"  disabled="disabled"  name="zl_pz"  id="zl_pz1"  class="yellowSty"  value="0"/></li>
				      <li><input type="text"  disabled="disabled"  name="zl_ss"  id="zl_ss1"  class="yellowSty" value="0"/></li>
				      
				   </ul>
				    <br/><br/>
				   <ul class="titleul2">
				      <li>实领辅料：</li>
				      <li><input type="text"  disabled="disabled"  name="zl_pz"  id="zl_pz2"  class="yellowSty" value="0"/></li>
				      <li><input type="text"  disabled="disabled"  name="zl_ss"  id="zl_ss2"  class="yellowSty" value="0"/></li>
				     
				   </ul>
				    <br/><br/>
				   <ul class="titleul2">
				      <li>虚退辅料：</li>
				      <li><input type="text"  name="zl_pz"  id="zl_pz3" class="yellowSty" value="0"/></li>
				      <li><input type="text"  name="zl_ss"  id="zl_ss3" class="yellowSty" value="0"/></li>
				     
				   </ul>
				    <br/><br/>
				   <ul class="titleul2">
				      <li>计算消耗：</li>
				      <li><input type="text"  disabled="disabled" id="zl_pz4" class="yellowSty  intf"  value=""/></li>
				      <li><input type="text" disabled="disabled"  id="zl_ss4"  class="yellowSty  intf" value=""/></li>
