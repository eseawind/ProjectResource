...
<script type="text/javascript" >
	$(function(){
		/**
		  说明： code是md_equipment表equipment_code
		     type是MD_EQP_CATEGORY表 code
		     目的：通过不同的菜单编号，判断是卷烟机，包装机，成型机
		...
		*/
		...
		if(code>0 && code<31){ //卷烟机
			...
			type=2;
			$.loadSelectData($("#eqpId"),"ALLPACKERS",false);
		}else if(code>=61 && code<=70){//装封箱机
			type=3;
			$.loadSelectData($("#eqpId"),"ALLBOXERS",false);
		}else if(code>=101 && code<=130){ //成型机
			type=4;
			$.loadSelectData($("#eqpId"),"ALLFILTERS",false);
		}
		...
		var bandParams=function(pageIndex,params,od,type){
			//$.post("${pageContext.request.contextPath}/wct/eqpRun/getEqpEfficiency.do?eqpCod=${loginInfo.equipmentCode}&pageIndex="+pageIndex+"&type="+type,params,function(obj){
			$.post("${pageContext.request.contextPath}/wct/eqpRun/getEqpEfficiency.do?eqpCod=&pageIndex="+pageIndex+"&type="+type,params,function(obj){				
		...
		$("#eqp-od").click(function(){
			type=1;
			//$('#search_form input[type!=button]').val(null);
			bandParams(1,params,order,type);
			typeName="机台";
		});
		$("#shift-od").click(function(){
			type=3;
			//$('#search_form input[type!=button]').val(null);
			bandParams(1,params,order,type);
			typeName="班次";
		});
		$("#team-od").click(function(){
			type=2;
			//$('#search_form input[type!=button]').val(null);
			bandParams(1,params,order,type);
			typeName="班组";
...
<div id="hisBadQty-title">设备有效作业率</div>
<div id="hisBadQty-seach-box" >
	<form id="search_form" style="margin-top: 5px;">
		<table width="100%" cellspacing="0" cellpadding="0">
			<tr>	
			<th style="width:70px;text-align:center;">机组</th>
			<td>
				<!-- <select name="eqpCod" id="eqpCod" style="height:20px;">
				</select> -->
				<select name="eqpId" id="eqpId" style="height:25px;">
				</select>
			</td>	
...
<div id="hisBadQty-page">
	<!-- <div style="float:left">
	      <input id="eqp-od" order="1" type="button"  value="机台" class="btn btn-default od"/>
	     <input id="shift-od" order="2" type="button"  value="班次" class="btn btn-default od"/>
	     <input id="team-od" order="3" type="button"  value="班组" class="btn btn-default od"/>
	</div> -->
