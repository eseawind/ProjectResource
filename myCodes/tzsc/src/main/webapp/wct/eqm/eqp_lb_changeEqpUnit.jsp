	...
	function showTable(list,index,pageCount){
	 ...
	 function saveTrouble(){
		 ...
		 html="<br><span color='#F0F0F0'>名称：</span>"
		 	...
		 	$("#ulSel").append(html);
		 	 /* var tranme=$("#trouble_name"+tf).val();
		 	var descriptionnn=$("#description"+tf).val();
		 	var uuseNum=$("#useNum"+tf).val();
		 	if(tf==0){
		 		 $("#ulSel").append(html); 
		 	}else if(tranme!=null&&tranme!=""&&descriptionnn!=null&&descriptionnn!=""&&uuseNum!=null&&uuseNum!=""&&tf!=0){
		 		 $("#ulSel").append(html); 
		 	}else{
			...
			//如果跟换备件  更新备件的数据
				if(flag==1&&tf==0){
					...
					//2、向备件更换记录表里面添加备件的更换信息    chooseid是EQM_MAINTAIN维修表记录里面的id里面包含设备id，负责人id，完成后会有完成人id
					 $.post("${pageContext.request.contextPath}/wct/eqm/checkplan/addFixRec.do",{all_id:all_id,use_n:use_n,all_num:all_num,trouble_id:myId,fix_type:0,type:type_});
					 
					 showAddFixResult(tf,flag);
				}else if(tf!=0&&flag==0){
					if(isPass){
						//手动添加备品备件记录
						$.post("${pageContext.request.contextPath}/wct/eqm/fixrec/addNewFix.do",{id:myId,fix_name:trouble_name,fix_code:description,useNum:useNum,type:type_})
						showAddFixResult(tf,flag);
					}else{
						jAlert("名称、型号和使用数量不准为空！","提示");
						return;
