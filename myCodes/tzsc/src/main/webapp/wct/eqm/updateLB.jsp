...
<script type="text/javascript">
	...
	var html2="";
	 $(function(){
		...
		$.post("${pageContext.request.contextPath}/wct/eqm/covelplan/queryLB.do?id="+id_,function(data){
			$("#remarks").val(data.remarks);
			$("#zlzl").val(data.jobName);
		},"Json");
	});
	 var codetest=[];
	 var nametest=[];
 ...
 //向文本域中添加选中的故障信息
 	...
 	function queryTroubleInfo(){
 	 ...
 	 $(function(){
  		var url="${pageContext.request.contextPath}/wct/eqm/eqp_lb_jsp.jsp?planId="+planId+"&&role="+role+"";
  		$("#zlid").attr("href",url);
  	}) 
 	...
 	//提交
	function updateCovelParam(){
 		var rema=$("#remarks").val();
 		jConfirm('是否确认修改?', '提示', function(r) {
	 			if(r){
	 				//更新维修类型和备注
	 				$.post("${pageContext.request.contextPath}/wct/eqm/covelplan/updateCovelParam.do",{paramId:id_,planId:planId,remarks:rema,fixType:0});
	 				//jAlert("数据修改成功！","提示");
	 				window.location.href="${pageContext.request.contextPath}/wct/eqm/eqp_lb_jsp.jsp?planId="+planId+"&&role="+role+"";
	 			}
 		})
	}
 	queryLB();
...
<table style="margin-left:15%">
	<tr>
		<td><font size="5"><b>名称</b></td>
		<td></td>
		<td><font size="5"><b>备注</b></td>
	</tr>
	<tr>
		<td><textarea id="zlzl"  rows="3" cols="21"  readonly="readonly" style="background-color: #F4F4F4; font-size:20px;"></textarea></td>
		<td></td>
		<td><textarea id="remarks"  rows="3" cols="21" style="background-color: #F4F4F4; font-size:20px;"></textarea></td>
	...
	<tr>
		<td><input type="button" id="update" onclick="updateCovelParam();" value="确认" style="background-color: #8F8F8F; width:100px;" class="btn btn-default"></td>
		...
		<td><a id="zlid"><input type="button"  value="返回" style="background-color: #8F8F8F; width:100px;" class="btn btn-default"/></a></td>
	</tr>
</table>
</div>
