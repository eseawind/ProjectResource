...
<script type="text/javascript">
	/*var arryKey、arryVal、qx_filter_key1、qx_filter_val1、csKey、csVal这些是公共的属性*/
				...
				tr.find("td").each(function(index){
					...
					if(index==5){//处理措施
	                    ...
	                    for(var i=0;i<unit_filter_key.length;i++ ){
	                        if(unit_filter_key[i]==oldText||unit_filter_val[i]==oldText){
	                        	se5+="<option value='"+unit_filter_key[i]+"' selected>"+unit_filter_val[i]+"</option>" ;
	                        }else{
	                        	se5+="<option value='"+unit_filter_key[i]+"' >"+unit_filter_val[i]+"</option>";
	                        }
	                    }
	                    se5+="</select>";
	                   $(this).html(se5);
				...
				if(data.rows!=null&&data.rows.length>0){
					for(var i=0;i<data.rows.length;i++){
						...
						for(var j=0;j<unit_filter_key.length;j++ ){
							if(unit_filter_key[j]==revalue.failureUom||unit_filter_val[j]==revalue.failureUom){
						...
						html+="<tr>"
							...
							+"	<td width='170px'>"+checkCondition+"</td>"	//存在问题
							+"	<td width='120px'>"+revalue.failureNum+"</td>"		//数量
							+"	<td width='62px'>"+failureUom+"</td>"		//单位
							+"	<td width='125px'>"+checkStep+"</td>"		//处理措施
							+"	<td width='96px'>"+addUserName+"</td>" //签名
							+"</tr>";
					}
				}else{}
	...
	function insert(){
		...
		var html="<tr>"
			+"	<td width='40px'>"
			+"		<input value='' type='checkbox' name='czg-tab-tbody-CK' id='czg-tab-tbody-CK'/></td>"
			+"	</td>"
			+"	<td width='40px'>"+(trLength+1)+"</td>"
			+"	<td width='130px'>"+nowDate+"</td>"
			+"	<td width='170px'>"+se3+"</td>"
			+"	<td width='120px'>"+se4+"</td>"
			+"	<td width='62px'>"+se5+"</td>"
			+"	<td width='125px'>"+se6+"</td>"
			+"	<td width='96px'>"+se7+"</td>"
			+"</tr>";
	           ...
	           var trCk = tr.find("#czg-tab-tbody-CK").get(0);//checkbox对象,当前第0列
	        	if(trCk!=null&&trCk!=undefined&&trCk.checked){
					 ...
					 if(tr.attr("edit")){//表示当前行都在 编辑状态
					 	...
					 	for(var j=0;j<unit_filter_key.length;j++ ){
							if(unit_filter_val[j]==td5){
								td5=unit_filter_key[j];
								break;
							 }
						}
						for(var j=0;j<csKey.length;j++ ){
	...
	<table border="1" borderColor="#9a9a9a"
		...
		<thead id="czg-tab-thead"
			...
			<tr>
				...
				<td width="170px">质量情况</td>
				<td width="120px">不合格数</td>
				<td width="62px">单位</td>
