...
<script type="text/javascript">
	...
	$(function(){
		 bandParams=function(pageIndex,params){
			...
			$.post("${pageContext.request.contextPath}/wct/msg/carn/getInfoWarnList.do?pageIndex="+pageIndex,params,function(reobj){
			...
			var list=reobj.rows;
				...
				$("#prodMailPlan-tab-tbody tr").each(function(idx){
						if(list.length>idx){
							...
							tr.find("td:eq(2)").html(revalue.title);
							tr.find("td:eq(3)").html(revalue.content);
							tr.find("td:eq(4)").html(revalue.time);
							tr.find("td:eq(5)").html(revalue.issuerName);
							tr.find("td:eq(6)").html("<input type='button' id='prodFault-time-order' value='确认'  onclick=completes('"+revalue.id+"') style='height:28px;width:60px;' class='btn btn-default'/>");
						}
				}); 
			},"JSON");
		};
	...
	function completes(id){
		$.post("${pageContext.request.contextPath}/wct/msg/carn/getInfoUpdate.do?id="+id,function(json){
			...
			<table border="1" borderColor="#9a9a9a" style="BORDER-COLLAPSE:collapse;font-size: 12px;font-weight: 500;" width="824" height="474" cellspacing="0" cellpadding="0">
				<thead id="prodMailPlan-tab-thead" style="background: #5a5a5a;color: #fff;">
					<tr>
						...
						<td class="t-header" style="width:120px">标题</td>
						<td class="t-header" style="width:320px">内容</td>
						<td class="t-header" style="width:80px">通知时间</td>
						<td class="t-header" style="width:120px">签发用户</td>
