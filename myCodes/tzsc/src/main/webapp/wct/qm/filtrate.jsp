...
<script type="text/javascript">
		...
		function recolums(){
			 $.ajax({
		     	   type: "POST",
		     	   url: '${pageContext.request.contextPath}/wctQm/queryClass.action?type=F',
		     	   data: " ",
		     	   async:false,
		...
		mydatagrid=$('#videoTable').datagrid({
			//height : 500,
			fit:true,
			url:'${pageContext.request.contextPath}/wctQm/queryQmSelfchItm.action?type=F',
			frozenColumns:[[{field:'name',title:'缺陷描述',width:100,align:'left',sortable:true,styler:function(value,rowData,rowIndex){ return 'background:#DDDDDD'}},]],
			columns :eval("("+arr+")"),
			toolbar:[{
	            text:'新增',
	            iconCls:'icon-add',
	            ...
	            handler:function(){
	              var datas=$("#videoTable").datagrid('getData').rows;
	    			  if(datas.length==0){
	    				...
	    				if(vallues!=""){
	    				var url="${pageContext.request.contextPath}/wctQm/addCount.action?type=F";
	    				$.ajax({
	    				   type: "POST",
	    				   url: url,
...
<div id="eqpRun-seach-box" style="background:#DDDDDD;">
	...
	<div data-options="region:'north',split:false,border:false" style="height:30px;background:#DDDDDD;">
		...
		<table width="100%" cellspacing="0" cellpadding="0" style="float: right;width: 370px;" >
			<tr>
				<!-- <td style="font-size:24px;font-weight:bold;">成型机请求</td> -->
				<td style="width:130px;">
					<label><input type="radio" checked="checked" name="cureqps" id="radio1" value="${loginGroup.eqps['filer1'].id}">${loginGroup.eqps['filer1'].cod}#成型机</label>
				</td>
				<td style="width:130px;">
					<label><input type="radio" name="cureqps" id="radio2" value="${loginGroup.eqps['filer2'].id}">${loginGroup.eqps['filer2'].cod}#成型机</label>
				</td>
