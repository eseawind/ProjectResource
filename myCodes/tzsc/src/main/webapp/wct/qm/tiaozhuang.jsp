...
<script type="text/javascript">
		var datagrid_tiaozhuang=null;
		$(function(){
			datagrid_tiaozhuang=$("#datagrid_tiaozhuang").datagrid({	
				frozenColumns:[[
				                {
									field : 'id',
									title : 'id',
		      ...
		      var editIndex = undefined;
		           ...
		           function endEditing(){
		               ...
		               if (datagrid_tiaozhuang.datagrid('validateRow', editIndex)){
		      
		                   datagrid_tiaozhuang.datagrid('endEdit', editIndex);
		              ...
		              function append(){
		            	  ...
		            	  if(datagrid_tiaozhuang.datagrid("getRows").length>0&&(datagrid_tiaozhuang.datagrid("getRows")[datagrid_tiaozhuang.datagrid("getRows").length-1]).jycx=="W"){
		            		  jAlert('已经尾检.', '系统提示');
		            		  return;
		            	  }
		            	  /* if(datagrid_tiaozhuang.datagrid("getRows")&&(datagrid_tiaozhuang.datagrid("getRows")[datagrid_tiaozhuang.datagrid("getRows").length-1]).jycx=="W"){
		            		  alert("已经尾检");
		            		  return;
		            	  } */
		                  ...
		                  if (endEditing()){
		                	  ...
		                	  if(datagrid_tiaozhuang.datagrid("getRows").length==0){
		                      ...
		                      datagrid_tiaozhuang.datagrid('appendRow',initcon);
		                      editIndex = datagrid_tiaozhuang.datagrid('getRows').length-1;
		                      datagrid_tiaozhuang.datagrid('selectRow', editIndex)
		              ...
		              function appendW(){
		            	  ...
		            	  if((datagrid_tiaozhuang.datagrid("getRows")[datagrid_tiaozhuang.datagrid("getRows").length-1]).jycx=="W"){
		            		  jAlert('已经尾检.', '系统提示');
		            		  return;
		            	  }
		              ...
		              function remove(){
		                  ...
		                  datagrid_tiaozhuang.datagrid('cancelEdit', editIndex)
		                          .datagrid('deleteRow', editIndex);
		                  editIndex = undefined;
		              }
		              function accept(){
		                  if (endEditing()){
		                      datagrid_tiaozhuang.datagrid('acceptChanges');
		                  }else{
		                	  jAlert('自检项不能为空', '系统提示');
		                	  return;
		                  }
		                  var curRow=datagrid_tiaozhuang.datagrid('getSelected');
		                  if(curRow){
			                  var wkCod=$("#wkcod").html();
			                  $.post("${pageContext.request.contextPath}/wctQm/addTzData.action",
			                		 {
			                	      "bean.id":curRow.id,
		              ...
		              function reject(){
		                  datagrid_tiaozhuang.datagrid('rejectChanges');
		                  editIndex = undefined;
		              }
		              function getChanges(){
		                  var rows = datagrid_tiaozhuang.datagrid('getChanges');
		              }
		              
		              /*编辑明细数据*/
		     	     function editRow(){
		    	 		 var curRow=datagrid_tiaozhuang.datagrid('getSelected');
		    	 		 var index=datagrid_tiaozhuang.datagrid('getRowIndex',curRow);
		    	         if(index==-1){
		    	        	 jAlert('请选择一行数据.', '系统提示');
		    	        	 return;
		    	         } 
		    	 		 if (editIndex != index){
		    	              if (endEditing()){
		    	            	  datagrid_tiaozhuang.datagrid('selectRow', index)
		    	    ...
		    	    var loadQmData=function(wkcod){
		    	    	datagrid_tiaozhuang.datagrid({
		    				url : "${pageContext.request.contextPath}/wctQm/listTzData.action",
		    	    ...
		    	    var deleteData=function(){
		    	    	...
		    	    	if(curRow){
		    	    		jConfirm("确认删除","系统提示",function(v){
		    	    			if(v){
		    	    				$.post("${pageContext.request.contextPath}/wctQm/delTzData.action",
...
<div id="eqpRun-title">包装工序条装质量自检</div>
...
<div class="easyui-layout" data-options="fit:true" style="border-radius: 4px;">
 <div data-options="region:'center',split:false,border:false" style="background:#DDDDDD;">  
        <div class="easyui-layout" data-options="fit:true,split:false,border:false" style="background:#DDDDDD;">  
            <div data-options="region:'center',split:false,border:false" style="background:#DDDDDD;">
            <table id="datagrid_tiaozhuang" style="background:#DDDDDD;" >
			</table>
            </div>  
        </div>  
    </div> 
