...
<script type="text/javascript">
					...
					.datagrid(
							{
								...
								queryParams : {
									...
									"qmSelfChkBean.wktyp":9,
									"qmSelfChkBean.lvl":$("#itemLvl").combobox("getValue"),
									"qmSelfChkBean.itemtype":$("#itemtyp").combobox("getValue")
		...
		function showErshi(){
			...
			$("#hid_div_ershi").css("display","block");
		}
		//20支烟支质量
		      var editIndex20 = undefined;
		           ...
		           function endEditing(){
		               ...
		               if (editIndex20 == undefined){
		            	   return true;
		               }
		      
		               if (ershi.datagrid('validateRow', editIndex20)){
		      
		                   ershi.datagrid('endEdit', editIndex20);
		...
		function appendWeight20(){
			if(isLogin!=1){
				...
				return;
		                 ...
		                 // if (endEditing()){
		                	 var seqno=0;
		                	  var rows=ershi.datagrid('getRows');
		                	  if(rows.length==0){
		                		  seqno=1;
		                	  }else{
		                		  seqno=rows[rows.length-1].seqno+1;
		                	  }
		                	  var initcon={seqno:seqno,val:0};
		                      ershi.datagrid('appendRow',initcon);
		                      editIndex20 = rows.length-1;
		                      ershi.datagrid('selectRow', editIndex20)
		                              .datagrid('beginEdit', editIndex20);
		                  //}
		                  $(".datagrid-editable-input").keyboard({
		      				layout:"1234567890.",
		    				customLayout: {
	   ...
	   function saveWeight20(){
		   if(isLogin!=1){
				jAlert("请先使用质检员用户名登录.","提示");
				return;
			}
		   ershi.datagrid('acceptChanges');		  
		   var ids = [];
		   var vals = [];			
		   var rows=ershi.datagrid('getRows');
		   for(var i=0;i<rows.length;i++){
			  ershi.datagrid('endEdit', i);
			  ids.push(rows[i].seqno);
			  vals.push(rows[i].val);
		   }	
			var ids=ids.join(",");
			var vals=vals.join(",");
			//alert(vals);
			$.post("${pageContext.request.contextPath}/wctQm/addWeight20.action",
					{"qmSelfChkBean.id":selfchk.datagrid('getSelected').id,
				     "itmIds":ids,
				     "vals":vals},function(v){
				if(v=="true"){
					selectWeight20();
					$("#itemtyp").combobox("setValue",1);					
					selectItmByBatch(1);
					jConfirm("操作成功,当前20支烟重量是否填写[组重缺陷]项?","提示", function(r) {
						if(r){
						    var rows=selfchk_det.datagrid("getRows");
							var idx=-1;
							for(var i=0;i<rows.length;i++){
								if(rows[i].cod=="Y-C27"){
									idx=i;
							...
							selfchk_det.datagrid('selectRow',idx); 
	   ...
	   function selectWeight20(){
		  // alert(selfchk.datagrid('getSelected').id);
		   ershi.datagrid(
					{
						url : "${pageContext.request.contextPath}/wctQm/list20Weight.action",
						queryParams : {
							"qmSelfChkBean.id" : selfchk.datagrid('getSelected').id
						}
					});
	   }
	   function deleteWeight20(){
		   if(isLogin!=1){
				jAlert("请先使用质检员用户名登录.","提示");
				return;
			}
			 var row= ershi.datagrid('getSelected');			
			 if(row){
				 if(row.id){
					 $.post("${pageContext.request.contextPath}/wctQm/deleteWeight20.action",{"weight.id" : row.id},function(v){
						    if(v=="true"){
						     selectWeight20();
							 jAlert("操作成功","提示");
						 	}else{
					 ...
					 ershi.datagrid("deleteRow",row.seqno-1);
				 }
			 }else{
				 jAlert("请选择一条检测批次","提示");
			 }
...
<div id="juanjie-title">${loginGroup.eqps['roller'].des}卷接/包装外观在线巡检</div>
...
<div class="easyui-layout" data-options="fit:true" style="border-radius: 4px;">
 <div data-options="region:'center',split:false,border:true" style="background:#DDDDDD;">  
        <div class="easyui-layout" data-options="fit:true,split:false">  
           <div data-options="region:'west',border:false,width:370"  style="background:#DDDDDD;">
            ...
            <div data-options="region:'south',split:false" style="height:150px">
	            <table id="ershi" style="background:#DDDDDD;" >
    ...
    <div id="selfchk_toolbar" style="height:28px;">
    	<table cellspacing="0" cellpadding="0">
	        <tr>
	           <td style="font-size:12px;color:#FFFFFF;font-weight:bold;padding:0 3px;">检验批次</td>
			    ...
			    <td >
					<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-remove',plain:true" onclick="delBatch();">删除</a>
			    </td>
			    <td style="width:57px">
			    </td> 
    	...
    	<table cellspacing="0" cellpadding="0">
	        <tr>
	           <td style="font-size:12px;color:#FFFFFF;font-weight:bold;padding:0 3px;">工段</td>
	            <td>
	           ...
	           <td style="font-size:12px;color:#FFFFFF;font-weight:bold;padding:0 3px;">外观等级</td>
	            <td>
	          		<select name="bean.lvl"
	          			id="itemLvl"
						class="easyui-combobox"
						data-options="panelHeight:'auto',editable:false,width:60">
    ...
    <!-- toolbar -->
    <div id="ershi_toolbar" style="height:28px;">
    	<table cellspacing="0" cellpadding="0">
	        <tr>
	           <td style="font-size:12px;color:#FFFFFF;font-weight:bold;padding:0 3px;">20支烟支质量</td>
				<td >
					<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true" onclick="appendWeight20();">新增</a>
			    </td>
			    <td >
					<a href="javascript:void(0);" class="easyui-linkbutton" 
					data-options="iconCls:'icon-save',plain:true,disabled:false" 
					onclick="saveWeight20();">保存</a>
			    </td>
			    <td >
					<a href="javascript:void(0);" class="easyui-linkbutton" 
					data-options="iconCls:'icon-remove',plain:true,disabled:false" 
					onclick="deleteWeight20();">删除</a>
			    </td>			    		   
	        </tr>
    	</table>
	</div>	
...
<!-- 登录hide div -->     
	<!-- <div id="hid_div_ershi"  style="display:none;height:768px;width:100%;top:0px;left:0px;position:absolute;z-index:90;" onclick="return false;">
		<div class="password_box" style="height:200px">
			<form >
			<div class="input-group" style="font-size:20px;">
			  二十支烟支质量(g)
			</div>
			<div class="input-group">
			  <span class="input-group-addon">样本1</span>
			  <input type="text" id="name" class="form-control keyboard" style="height: 30px;;border-radius:5px" />
			</div>
			<div class="input-group">
			  <span class="input-group-addon">样本2</span>
			  <input type="password" id="pwd" class="form-control keyboard" style="height: 30px;border-radius:5px" />
			</div>
			<div class="input-group">
			  <span class="input-group-addon">样本3</span>
			  <input type="password" id="pwd" class="form-control keyboard" style="height: 30px;border-radius:5px" />
			</div>
		<div class="password_botton">			
			<input id="sub" type="button" value="保存" onclick="saveErshi()" class="btn btn-default"/>			
			<input id="cancel" type="button" value="取消" onclick="javascript:$('#hid_div_ershi').css('display','none');" class="btn btn-default"/>
		</div>
	</form>
	</div>
	</div> -->
