...
<script type="text/javascript">
			...
			$("#gczj-tab-tbody tr").die().live("click",function(){
				...
				tr.find("td").each(function(index){
	                ...
	                if(index==3){
				        ...
				        var classKey=".zl-keyboard-"+tr.find("td:eq(1)").text()+"td"+index;
				        var classIds =formId+classId;
	                	var se3="<input type='text' onkeyup='checkInput(this)' class='"+classIds+"' style='width:100%;height:30px;' value='"+oldText+"'/>";
	                   	$(this).html(se3);
	                   	$(classKey).keyboard({
	        				layout:"1234567890:-",
	        				customLayout: {
					...
					if(index==4){//外观质量情况
	                    ...
	                    for(var i=0;i<qx_pack_tzKey.length;i++ ){
	                        if(qx_pack_tzKey[i]==oldText||qx_pack_tzVal[i]==oldText){
	                        	se4+="<option value='"+qx_pack_tzKey[i]+"' selected>"+qx_pack_tzVal[i]+"</option>" ;
	                        }else{
	                        	se4+="<option value='"+qx_pack_tzKey[i]+"' >"+qx_pack_tzVal[i]+"</option>";
	                        }
	                    }
	                    se4+="</select>";
	                   $(this).html(se4);
					...
					/*if(index==6){
	              		if(zjId==""||zjId==undefined){//表示是 新增的行
	              		 ...
	              		 var se6="<select style='width:100%;'>";
					...
					if(index==7){
					    ...
					    var classKey=".zl-keyboard-"+index+"td"+index;
					    var classIds =formId+classId;
						var se7="<input type='text' class='"+classIds+"' onkeyup='checkInput(this)' style='width:100%;height:30px;' value='"+oldText+"'/>";
	                   	$(this).html(se7);
	                    $(classKey).keyboard({
	        				layout:"1234567890.-",
	        				customLayout: {
					...
					if(index==8){
	                    ...
	                    var se8="<select style='width:100%;height:30px;'>";
	                    for(var i=0;i<unit_key.length;i++ ){
	                        if(unit_key[i]==oldText||unit_val[i]==oldText){
	                        	se8+="<option value='"+unit_key[i]+"' selected>"+unit_val[i]+"</option>" ;
	                        }else{
	                        	se8+="<option value='"+unit_key[i]+"' >"+unit_val[i]+"</option>";
	                        }
	                    }
	                    se8+="</select>";
	                   $(this).html(se8);
						 ...
						 var se9="<select style='width:100%;height:30px;'>";
		                    for(var i=0;i<standkey.length;i++ ){
		                        if(standkey[i]==oldText || standval[i]==oldText){
		                        	se9+="<option value='"+standval[i]+"' selected>"+standkey[i]+"</option>" ;
		                        }else{
		                        	se9+="<option value='"+standval[i]+"' >"+standkey[i]+"</option>";
		                        }
		                 }
		                 se9+="</select>";
		                 $(this).html(se9); 
	...
	function gczjSearch(){
		...
		var params ={proWorkId : schWorkorderId,processType:'C'}//工单主键ID
		$.post("${pageContext.request.contextPath}/wct/massProcess/getList.do",params,function(data){
				...
				if(data.rows!=null&&data.rows.length>0){
					for(var i=0;i<data.rows.length;i++){
						...
						for(var j=0;j<qx_pack_tzKey.length;j++ ){//外观质量情况
							if(qx_pack_tzKey[j]==revalue.runCondition||qx_pack_tzVal[j]==revalue.runCondition){
								runCondition=qx_pack_tzVal[j];
								 break;
							 }
						} 
						...
						html+="<tr>"
							...
							+"	<td width='60px'>"+revalue.shortTime+"</td>"
							...
							+"	<td width='50px'>"+revalue.runRemark+"</td>"
							+"</tr>";
					}
				}else{}
	...
	function gczjInsert(){
        ...
        /* var se6="<select width=30px>";
         ...
         se7+="</select>";
  	 	...
  	 	var se8="<input type='text' width='40px' value='0'></input>";
	        	...
	        	if(trCk!=null&&trCk!=undefined&&trCk.checked){
					 ...
					 if(tr.attr("edit")){//表示当前行都在 编辑状态
						...
						// td10 = tr.find("td:eq(10)").text();//这列不可以修改
						 //根据中文找 key
					 	for(var j=0;j<qx_pack_tzKey.length;j++ ){//外观质量情况 select
							if(qx_pack_tzVal[j]==td4){
								td4=qx_pack_tzKey[j];
								 break;
							 }
						}
						...
						for(var j=0;j<unit_key.length;j++ ){//数量单位    select
							if(unit_val[j]==td7){
								td7=unit_key[j];
								 break;
							 }
