        ...
        changeClick=function(type){
			...
			$("#czg-tab-tbody tr").die().live("click",function(){
				...
				tr.find("td").each(function(index){
	            	...
	            	//根据key 或 value 赋值
	            	/* if(index==2){
	            		var se2="<input style='width:100px;' value='"+oldText+"''/>";
	                   $(this).html(se2);
	            	} */
	            	if(index==2){
	            	    ...
	            	    var se2="<input type='text' class='"+classIds+"' onkeyup='checkInput(this)' style='width:100px;height:30px;' value='"+oldText+"' />";
	                    $(this).html(se2);
	                    $(classKey).keyboard({
	        				layout:"1234567890:-",
	        				customLayout: {
					...
					if(index==4){//质量情况
	                    ...
	                    for(var i=0;i<qx_pack_key.length;i++ ){
	                        if(qx_pack_key[i]==oldText||qx_pack_val[i]==oldText){
	                        	se4+="<option value='"+qx_pack_key[i]+"' selected>"+qx_pack_val[i]+"</option>" ;
	                        }else{
	                        	se4+="<option value='"+qx_pack_key[i]+"' >"+qx_pack_val[i]+"</option>";
	                        }
	                    }
	                    se4+="</select>";
	                   $(this).html(se4);
					...
					if(index==5){
	                    ...
	                    var se5="<input type='text' class='"+classIds+"' onkeyup='checkInput(this)' style='width:100px;height:30px;' value='"+oldText+"' />";
	                	$(this).html(se5);
	                	$(classKey).keyboard({
	        				layout:"1234567890",
	        				customLayout: {
					...
					if(index==6){//处理措施
	                    ...
	                    for(var i=0;i<unit_key.length;i++ ){
	                        if(unit_key[i]==oldText||unit_key[i]==oldText){
	                        	se6+="<option value='"+unit_key[i]+"' selected>"+unit_val[i]+"</option>" ;
	                        }else{
	                        	se6+="<option value='"+unit_key[i]+"' >"+unit_val[i]+"</option>";
	                        }
	                    }
	                    se6+="</select>";
	                   $(this).html(se6);
				...
				if(data.rows!=null&&data.rows.length>0){
					for(var i=0;i<data.rows.length;i++){
						...
						for(var j=0;j<unit_key.length;j++){
							if(unit_key[j]==revalue.failureUom||unit_key[j]==revalue.failureUom){
...
<title>操作工_1</title>
	...
	<table border="1" borderColor="#9a9a9a"
		...
		<input type="hidden"  id="tfRole" value="1"/>
	</table>
</html>
