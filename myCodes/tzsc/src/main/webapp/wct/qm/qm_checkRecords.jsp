...
<script type="text/javascript">
	...
	$(function(){
	    ...
	    $("input[name='checkTime']").val(s2);	//时间用这个
	});
	...
	$(function(){
		var bandParams=function(pageIndex,params){
			$.post("${pageContext.request.contextPath}/wct/qm/qmRecord/getSummaryList.do?pageIndex="+pageIndex,params,function(v){				
				var reobj=eval("("+v+")");
				var list=reobj.rows;
				...
				$("#prodMailPlan-tab-tbody tr").each(function(idx){
						if(list.length>idx){
							...
							tr.find("td:eq(1)").html(revalue.id);//主键ID 
							tr.find("td:eq(2)").html(revalue.rollerCode);//机台
							tr.find("td:eq(3)").html(revalue.matName);//牌号
							tr.find("td:eq(4)").html(revalue.scScore);//得分
							tr.find("td:eq(5)").html(revalue.scLevel);//质量水平
						    tr.find("td:eq(6)").html("<input type='button' id='prodFault-time-order' value='扣分详细'  onclick=completes('"+revalue.id+"') style='height:28px;width:60px;' class='btn btn-default'/>"); 
						}
				});
			});
		};
		...
		$("#prodMailPlan-reset").click(function(){
			params={};
			$("#prodMailPlan-wct-frm input[type!=button]").val(null);
			$("#matprod").val(""); 
			$("#prodStatus").val("2"); //默认值 供后台用
		});
		//加载品名
	...
	function completes(id){
		var path ="${pageContext.request.contextPath}/wct/qm/qm_checkRecordsParams.jsp?pid="+id;
...
<body>
			...
			<form id="prodMailPlan-wct-frm">
				...
				<table width="100%" cellspacing="0" cellpadding="0">
					<tr>
						<td>日期</td>
						<td>
							<input type="text" name="checkTime" class="mh_date" readonly="readonly" style="width:110px;height:20px;"/>
						</td>
						<td>班次</td>
						<td>
			...
			<table border="1" borderColor="#9a9a9a" style="BORDER-COLLAPSE:collapse;font-size: 12px;font-weight: 500;" width="824" height="474" cellspacing="0" cellpadding="0">
				<thead id="prodMailPlan-tab-thead" style="background: #5a5a5a;color: #fff;">
					<tr>
						...
						<td class="t-header" style="width:90px">机台</td>
						<td class="t-header" style="width:110px">牌号</td>
						<td class="t-header" style="width:60px">综合得分</td>
						<td class="t-header" style="width:110px">质量水平</td>
						<td class="t-header" style="width:80px">详细</td>
					</tr>
				</thead>
	...
	<div id="mat_system">
		<div >
			<iframe id="insertWeb" frameborder="0"  style="height:522px;width: 100%;padding-top: -20px;">
			</iframe>				
		</div>
		<div>
