...
<script type="text/javascript">
	...
	$(function(){
		var bandParams=function(pageIndex,params){
			$.post("${pageContext.request.contextPath}/wct/qm/prod/getList.do?pageIndex="+pageIndex,params,function(v){				
				var reobj=eval("("+v+")");
				var list=reobj.rows;
				...
				$("#prodMailPlan-tab-tbody tr").each(function(idx){
						if(list.length>idx){
							...
							tr.find("td:eq(3)").html(revalue.prodNumber);
							tr.find("td:eq(4)").html(revalue.fileName);//文件名称
							tr.find("td:eq(5)").html(revalue.fileType);//文件类型
							tr.find("td:eq(6)").html(revalue.uploadUrl);//路径 
						    tr.find("td:eq(7)").html("<input type='button' id='prodFault-time-order' value='预览'  onclick=completes('"+revalue.uploadUrl+"') style='height:28px;width:60px;' class='btn btn-default'/>"); 
						}
				});
			}); 
		};
			...
			<form id="prodMailPlan-wct-frm">
				...
				<table width="100%" cellspacing="0" cellpadding="0">
					<tr>
						<td>品名</td>
						<td>
							<select style="width:110px;height:20px;"  name="id" id="matprod" >
				        	</select>
						</td>
						<td>编号</td>
						<td>
							<input type="text" placeholder="请输编号" class="" name="prodNumber"style="width: 110px; height: 20px;" />
						</td>
						<td>文件名</td>
						<td>
							<input type="text" placeholder="请输文件名" class="" name="fileName"style="width: 110px; height: 20px;" />
						</td>
			...
			<table border="1" borderColor="#9a9a9a" style="BORDER-COLLAPSE:collapse;font-size: 12px;font-weight: 500;" width="824" height="474" cellspacing="0" cellpadding="0">
				<thead id="prodMailPlan-tab-thead" style="background: #5a5a5a;color: #fff;">
					<tr>
						...
						<td class="t-header" style="width:110px">编号</td>
