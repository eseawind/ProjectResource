...
<script type="text/javascript">
				...
				tr.find("td").each(function(index){
					...
					if(index==6){//处理措施
	                    ...
	                    for(var i=0;i<sealer_unit_key.length;i++ ){
	                        if(sealer_unit_key[i]==oldText||sealer_unit_key[i]==oldText){
	                        	se6+="<option value='"+sealer_unit_key[i]+"' selected>"+sealer_unit_val[i]+"</option>" ;
	                        }else{
	                        	se6+="<option value='"+sealer_unit_key[i]+"' >"+sealer_unit_val[i]+"</option>";
	                        }
	                    }
	                    se6+="</select>";
	                   $(this).html(se6);
				...
				if(data.rows!=null&&data.rows.length>0){
					for(var i=0;i<data.rows.length;i++){
						...
						for(var j=0;j<sealer_unit_key.length;j++){
							if(sealer_unit_key[j]==revalue.failureUom||sealer_unit_key[j]==revalue.failureUom){
						...
						html+="<tr>"
							...
							+"	</td>"
							+"	<td width='52px'>"+(i+1)+"</td>"
							+"	<td width='90px'>"+revalue.checkTime+"</td>"
							+"	<td width='128px'>"+checkMatter+"</td>"
							+"	<td width='186px'>"+checkCondition+"</td>"
							+"	<td width='105px'>"+revalue.failureNum+"</td>"
							+"	<td width='53px'>"+failureUom+"</td>"
							+"	<td width='130px'>"+checkStep+"</td>"
							+"</tr>";
					}
				}else{}
	...
	function insert(){
		...
		var html="<tr>"
			...
			+"	</td>"
			+"	<td width='52px'>"+(trLength+1)+"</td>"
			+"	<td width='90px'>"+nowDate+"</td>"
			+"	<td width='128px'>"+se3+"</td>"
			+"	<td width='186px'>"+se4+"</td>"
			+"	<td width='105px'>"+se5+"</td>"
			+"	<td width='53px'>"+se6+"</td>"
			+"	<td width='130px'>"+se7+"</td>"
			+"</tr>";
	           ...
	           var trCk = tr.find("#czg-tab-tbody-CK").get(0);//checkbox对象,当前第0列
	        	if(trCk!=null&&trCk!=undefined&&trCk.checked){
					 ...
					 if(tr.attr("edit")){//表示当前行都在 编辑状态
					 	...
					 	for(var j=0;j<qx_carton_key.length;j++ ){
							if(qx_carton_val[j]==td4){
								td4=qx_carton_key[j];
								break;
							 }
						}
						for(var j=0;j<sealer_unit_key.length;j++ ){
							if(sealer_unit_val[j]==td6){
								td6=sealer_unit_key[j];
								break;
							 }
						}
						for(var j=0;j<csKey.length;j++ ){
	...
	<table border="1" borderColor="#9a9a9a"
		...
		<thead id="czg-tab-thead"
			...
			<tr>
				...
				<td width="52px">序号</td>
				<td width="90px">时间</td>
				<td width="128px"><p>首检原因</p></td>
				<td width="186px">质量情况</td>
				<td width="105px">不合格数</td>
				<td width="53px">单位</td>
				<td width="130px">处理措施</td>
			</tr>
		</thead>
