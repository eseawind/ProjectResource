...
<script type="text/javascript">
	/*var arryKey、arryVal、qx_carton_key、qx_carton_val、csKey、csVal这些是公共的属性*/
	                ...
	                if(index==3){
	                    ...
	                    for(var i=0;i<qx_carton_key.length;i++ ){
	                    	 if(qx_carton_key[i]==oldText||qx_carton_val[i]==oldText){
	                        	se3+="<option value='"+qx_carton_key[i]+"' selected>"+qx_carton_val[i]+"</option>" ;
	                        }else{
	                        	se3+="<option value='"+qx_carton_key[i]+"' >"+qx_carton_val[i]+"</option>";
	                        }
	                    }
	                    se3+="</select>";
	                   $(this).html(se3);
	            		 ...
	            		 var formId="form-control ";
				            ...
				            var classIds =formId+classId;
		              		var se4="<input type='text' value='"+oldText+"' " 
		                    	+" class='"+classIds+"' "
		                    	+"style='height: 30px;width:100%;height:30px;border-radius:5px'/>";
		                	$(this).html(se4);
		                	$(classKey).keyboard({
		        				layout:"1234567890",
		        				customLayout: {
					...
					if(index==5){//处理措施
	                    ...
	                    for(var i=0;i<sealer_unit_key.length;i++ ){
	                        if(sealer_unit_key[i]==oldText||sealer_unit_val[i]==oldText){
	                        	se5+="<option value='"+sealer_unit_key[i]+"' selected>"+sealer_unit_val[i]+"</option>" ;
	                        }else{
	                        	se5+="<option value='"+sealer_unit_key[i]+"' >"+sealer_unit_val[i]+"</option>";
	                        }
	                    }
	                    se5+="</select>";
	                   $(this).html(se5);
		...
		$.post("${pageContext.request.contextPath}/wct/massFrist/getList.do",params,function(data){
				...
				if(data.rows!=null&&data.rows.length>0){
					for(var i=0;i<data.rows.length;i++){
						...
						for(var j=0;j<qx_carton_key.length;j++ ){
							if(qx_carton_key[j]==revalue.checkCondition||qx_carton_val[j]==revalue.checkCondition){
								checkCondition=qx_carton_val[j];
								 break;
							}
						}
						for(var j=0;j<sealer_unit_key.length;j++ ){
							if(sealer_unit_key[j]==revalue.failureUom||sealer_unit_val[j]==revalue.failureUom){
								failureUom=sealer_unit_val[j];
								 break;
							 }
						}
						...
						html+="<tr>"
							...
							+"	<td width='130px'>"+checkTime+"</td>"
							+"	<td width='198px'>"+checkCondition+"</td>"	//存在问题
							+"	<td width='100px'>"+revalue.failureNum+"</td>"		//数量
							+"	<td width='52px'>"+failureUom+"</td>"		//单位
							+"	<td width='125px'>"+checkStep+"</td>"		//处理措施
							+"	<td width='75px'>"+addUserName+"</td>" //签名
							+"</tr>";
					}
				}else{}
	...
	function insert(){
		...
		var html="<tr>"
			...
			+"	</td>"
			+"	<td width='60px'>"+(trLength+1)+"</td>"
			+"	<td width='130px'>"+nowDate+"</td>"
			+"	<td width='198px'>"+se3+"</td>"
			+"	<td width='100px'>"+se4+"</td>"
			+"	<td width='52px'>"+se5+"</td>"
			+"	<td width='125px'>"+se6+"</td>"
			+"	<td width='75px'>"+se7+"</td>"
			+"</tr>";
	           ...
	           var trCk = tr.find("#czg-tab-tbody-CK").get(0);//checkbox对象,当前第0列
	        	if(trCk!=null&&trCk!=undefined&&trCk.checked){
					 ...
					 if(tr.attr("edit")){//表示当前行都在 编辑状态
						 ...
						 //根据中文找 key
					 	for(var j=0;j<qx_carton_key.length;j++ ){
							if(qx_carton_val[j]==td3){
								td3=qx_carton_key[j];
								 break;
							 }
						}
					 	for(var j=0;j<sealer_unit_key.length;j++ ){
							if(sealer_unit_val[j]==td5){
								td5=sealer_unit_key[j];
								break;
							 }
						}
						for(var j=0;j<csKey.length;j++ ){
	...
	<table border="1" borderColor="#9a9a9a"
		...
		<thead id="czg-tab-thead"
			...
			<tr>
				...
				<td width="198px">质量情况</td>
				<td width="100px">不合格数</td>
				<td width="52px">单位</td>
				<td width="125px">处理措施</td>
				<td width="75px">签字</td>
			</tr>
		</thead>
