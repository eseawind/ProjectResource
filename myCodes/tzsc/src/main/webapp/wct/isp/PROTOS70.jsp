    ...
    function getRollerData(){
		$.post("${pageContext.request.contextPath}/wct/isp/roller/getRollerData.do",{"equipmentCode":"${loginInfo.equipmentCode}"},function(json){
			...
			/* if(qty!=null && qty >0 ){
				...
				var rand3=Math.random()*0.1;
				var o_lb=parseFloat($("#lvbangVal").html());
			    var o_jyz=parseFloat($("#juanyanzhiVal").html());
			    var o_ssz=parseFloat($("#shuisongzhiVal").html());
				if(lvbangVal==0){
					lvbangVal=(qty*(lvbangBzdh+rand1)).toFixed(2);
					if(lvbangVal<o_lb){
						lvbangVal=o_lb;
					}
				}
				if(juanyanzhiVal==0){
					juanyanzhiVal=(qty*(juanyanzhiBzdh+rand2)).toFixed(2);
					if(juanyanzhiVal<o_jyz){
						juanyanzhiVal=o_jyz;
					}
				}
				if(shuisongzhiVal==0){
					shuisongzhiVal=(qty*(shuisongzhiBzdh+rand3)).toFixed(2);
					if(shuisongzhiVal<o_ssz){
						shuisongzhiVal=o_ssz;
					}
				}
			} */
			...
			if(lvbangBzdh!=0){
				juanyanzhiVal=(lvbangVal/lvbangBzdh * juanyanzhiBzdh).toFixed(2);
				shuisongzhiVal=(lvbangVal/lvbangBzdh * shuisongzhiBzdh).toFixed(2);
			}
...
<div class="info_machine">
	...
	<div class="info_machine_left fl">
		...
		<div class="fr machine_info">
			<p>设备型号：<span id="roller-zpj-sbxh">-</span>
			<p>装盘数量：<span id="roller-zpj-edcs">-</span>盘</p>
		</div>	
	</div>
	<!-- 主机 -->
	...
	<div class="info_machine_right fr">
		...
		<div class="fr machine_info">
			<p>设备型号：<span id="roller-zb-sbxh">-</span>
			<p>接收数量：<span id="roller-zb-edcs">-</span>万支</p>
		</div>	
	</div>
</div>
<div class="info_area">
