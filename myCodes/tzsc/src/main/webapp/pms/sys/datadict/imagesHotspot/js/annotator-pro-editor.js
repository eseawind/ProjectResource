;(function ( $, window, document, undefined ) {
	...
	var annotation_defaults = {
        tint_color : "#000000",//tint_color : "#000000",
        style : 4,
        popup_width : "auto",
        popup_height : "auto",
        popup_position : "right",//top
        ...
        html : "",
        id : "my-annotation-",
        spot_sys_eqp_type_id : "",//自定义属性 add by luther.zhang 20150721
        spot_left : 0,
        spot_top : 0,
        spot_width : 44,
        spot_height : 44,
	...
	function form_events() {
		...
		//点击保存方法
		$('#radio-editor-mode-save-label').on('click', function() {
			apply_settings();
			generate_jquery();
			var result = $('#well-jquery').text();
			if(result.length==35){//如果不设置,默认的长度是 35
				alert("请先选择图片区域");
			}else{
				//alert(result);
				var url = $('#contextPath').val()+"/pms/sys/eqptype/setUpImage.do";
				//保存到后台
				var filePath = $('#filePath').val();//图片 上传路径
				//alert(filePath);
				$.post(url,{id :$('#twoRowId').val(),result:result,filePath:filePath}, function(json) {
					if (json.success) {
						alert(json.msg);
						window.opener.queryMdType();//调用父窗口方法
					}else{
						alert(json.msg);
					}
				},'JSON');
	...
	function generate_jquery() {
		...
		for (var i=0; i<annotations.length; i++) {
			...
			//自定义属性 add by luther.zhang 20150721
			result += "<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;spot_sys_eqp_type_id : " + '"' + $('#twoRowId').val() + '"' + ',';
			//end
	...
	function generate_preview() {
		...
		for (var i=0; i<annotations.length; i++) {
			...
			var annotationsId= annotations[i].id;
			var spot_sys_eqp_type_id = annotations[i].spot_sys_eqp_type_id;//自定义属性 add by luther.zhang 20150721
			annotations[i] = {
				tint_color : annotations[i].tint_color,
				style : annotations[i].style,//点 圈圈样式
				popup_width : annotations[i].popup_width,
				popup_height : annotations[i].popup_height,
				popup_position : annotations[i].popup_position,//上下左右 显示样式
				content_type : annotations[i].content_type,//内容样式
				//title : annotations[i].title,
				//text :  annotations[i].text, 
				//通过给 id赋值 可以传值，对方只需要 对象.id 即可取值
				title : "<a id='"+spot_sys_eqp_type_id+"' href='javascript:;' style='color:#ffffff;text-decoration:none;' onclick='onjqueryClick(this);'>"+annotations[i].title+"</a>",
				text :  "<a id='"+spot_sys_eqp_type_id+"' href='javascript:;' style='color:#ffffff;text-decoration:none;' onclick='onjqueryClick(this);'>"+annotations[i].text+"</a>",
				text_color : annotations[i].text_color,
				//html : annotations[i].html,
				html : "<a id='"+spot_sys_eqp_type_id+"' href='javascript:;' style='color:#ffffff;text-decoration:none;' onclick='onjqueryClick(this);'>"+annotations[i].html+"</a>",
				id : annotationsId,
				spot_left : spot_left,
				spot_top : spot_top,
			...
			init_canvas(canvasWidth, canvasHeight, function() {
				...
				if (options.annotations != undefined) {
					for (var i=0; i<options.annotations.length; i++) {
						...
						drawable_settings.spot_sys_eqp_type_id=annotation.spot_sys_eqp_type_id;//自定义属性 add by luther.zhang 20150721
	...
	$(document).ready(function() {
		...
		init_canvas(0, 0, function() {
			init_global_events();//鼠标事件
			form_events();//form表单里控件 事件
			load_settings({});//加载 设置
			//add by luther.zhang 20150717
			var contextDes = $('#contextDes').val();
			//var contextPath = $('#contextPath').val();
			if(contextDes!=null&&contextDes!=""){
				load_jquery(contextDes);//加载信息
				//alert(contextDes);
			}
			//end
			refresh_form();//控制 隐藏显示
		});
	});
