...
<script type="text/javascript">
		...
		matPickGrid.datagrid({
			url : "${pageContext.request.contextPath}/pms/eqmChange/queryList.do",
		...
		function saveRow(){
			...
			var rows = matPickGrid.datagrid('getRows');
			var checkboxs  = document.getElementsByName("changeId");//所有checkbox
			 var reqString ='['; 
			 var isChecked=false; 
			for(var i=0;i<rows.length;i++){//所有的 checkboxs 
				if(checkboxs[i].checked){
					...
					var changeDate = objBean.changeDate;
			...
			$.post("${pageContext.request.contextPath}/pms/eqmChange/addOrEditBean.do",pams,function(json){
				if (json.success) {
		...
		function deleteRow(){
			...
			$.post("${pageContext.request.contextPath}/pms/eqmChange/deleteById.do",pams,function(json){
				if (json.success) {
					$.messager.show('提示','操作成功', 'info');
