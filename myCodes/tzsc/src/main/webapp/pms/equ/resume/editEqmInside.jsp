...
<script type="text/javascript">
		var matPickGrid;
		var editRow = undefined; //定义全局变量：当前编辑的行 ok
		...
		matPickGrid.datagrid({
			url : "${pageContext.request.contextPath}/pms/eqmInside/queryList.do",
		...
		function saveRow(){
			...
			var rows = matPickGrid.datagrid('getRows');
			var checkboxs  = document.getElementsByName("insideId");//所有checkbox
			 var reqString ='['; 
			 var isChecked=false; 
			for(var i=0;i<rows.length;i++){//所有的 checkboxs 
				if(checkboxs[i].checked){
					...
					var insideDate = objBean.insideDate;
					if(insideDate==null||insideDate==''){
						$.messager.show('提示','年月日不可为空', 'info');
						return false;
					}
					reqString +=jsonBean+',';
				}
			...
			$.post("${pageContext.request.contextPath}/pms/eqmInside/addOrEditBean.do",pams,function(json){
				if (json.success) {
		...
		function deleteRow(){
			...
			$.post("${pageContext.request.contextPath}/pms/eqmInside/deleteById.do",pams,function(json){
				if (json.success) {
					$.messager.show('提示','操作成功', 'info');
