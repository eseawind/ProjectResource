...
<script type="text/javascript">
		...
		matPickGrid.datagrid({
			url : "${pageContext.request.contextPath}/pms/eqmStop/queryList.do",
		...
		function saveRow(){
			...
			var rows = matPickGrid.datagrid('getRows');
			var checkboxs  = document.getElementsByName("stopId");//所有checkbox
			 var reqString ='['; 
			 var isChecked=false; 
			for(var i=0;i<rows.length;i++){//所有的 checkboxs 
				if(checkboxs[i].checked){
					...
					var stopTime = objBean.stopTime;
					if(stopTime==null||stopTime==''){
						$.messager.show('提示','停用时间不可为空', 'info');
						return false;
					}
					reqString +=jsonBean+',';
				}
			...
			$.post("${pageContext.request.contextPath}/pms/eqmStop/addOrEditBean.do",pams,function(json){
				if (json.success) {
		...
		function deleteRow(){
			...
			$.post("${pageContext.request.contextPath}/pms/eqmStop/deleteById.do",pams,function(json){
				if (json.success) {
					$.messager.show('提示','操作成功', 'info');
