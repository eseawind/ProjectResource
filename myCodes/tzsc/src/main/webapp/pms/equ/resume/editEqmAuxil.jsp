...
<script type="text/javascript">
		...
		$(function() {
			matPickGrid = $('#matPickGrid').datagrid({
				...
				columns : [ [ {
					...
					width : 100,
					sortable : true
					,editor: { type: 'text', options: { required: false} } //validatebox
				} , {
					field : 'auxilUnit',
					title : '单位',
					width : 110,
		...
		matPickGrid.datagrid({
			url : "${pageContext.request.contextPath}/pms/eqmAuxil/queryList.do",
		...
		function addRow(){
			...
			if (editRow == undefined) {
				...
				matPickGrid.datagrid('beginEdit', 0);
                editRow = 0;
                */
			}
		}
		...
		function saveRow(){
			...
			var checkboxs  = document.getElementsByName("auxilId");//所有checkbox
			 var reqString ='['; 
			 var isChecked=false; 
			for(var i=0;i<rows.length;i++){//所有的 checkboxs 
				if(checkboxs[i].checked){
					...
					var auxilName = objBean.auxilName;
					if(auxilName==null||auxilName==''){
						$.messager.show('提示','名称不可为空', 'info');
						return false;
					}
					reqString +=jsonBean+',';
				}
			...
			$.post("${pageContext.request.contextPath}/pms/eqmAuxil/addOrEditBean.do",pams,function(json){
				if (json.success) {
		...
		function deleteRow(){
			...
			for(var i=0;i<rows.length;i++){//所有的 checkboxs 
				if(checkboxs[i].checked){
					...
					var jsonBean = JSON.stringify(rows[i]);
					var objBean = eval( "(" + jsonBean + ")" );//转换后的JSON对象
					var auxilId = objBean.auxilId;
					if(auxilId!=undefined){
						deleteIds +=auxilId+",";
					}
				}
			}
			...
			$.post("${pageContext.request.contextPath}/pms/eqmAuxil/deleteById.do",pams,function(json){
				if (json.success) {
					$.messager.show('提示','操作成功', 'info');
