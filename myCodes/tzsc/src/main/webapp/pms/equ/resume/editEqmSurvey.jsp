...
<script type="text/javascript">
		...
		matPickGrid.datagrid({
			url : "${pageContext.request.contextPath}/pms/eqmSurvey/queryList.do",
		...
		function saveRow(){
			...
			var rows = matPickGrid.datagrid('getRows');
			var checkboxs  = document.getElementsByName("surveyId");//所有checkbox
			 var reqString ='['; 
			 var isChecked=false; 
			for(var i=0;i<rows.length;i++){//所有的 checkboxs 
				if(checkboxs[i].checked){
					...
					var surveyDate = objBean.surveyDate;
					if(surveyDate==null||surveyDate==''){
						$.messager.show('提示','调出日期不可为空', 'info');
						return false;
					}
					reqString +=jsonBean+',';
				}
			...
			$.post("${pageContext.request.contextPath}/pms/eqmSurvey/addOrEditBean.do",pams,function(json){
				if (json.success) {
		...
		function deleteRow(){
			...
			$.post("${pageContext.request.contextPath}/pms/eqmSurvey/deleteById.do",pams,function(json){
				if (json.success) {
					$.messager.show('提示','操作成功', 'info');
