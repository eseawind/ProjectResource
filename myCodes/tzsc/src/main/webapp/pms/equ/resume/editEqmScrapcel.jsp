...
<script type="text/javascript">
		...
		matPickGrid.datagrid({
			url : "${pageContext.request.contextPath}/pms/eqmScrapcel/queryList.do",
		...
		function saveRow(){
			...
			var rows = matPickGrid.datagrid('getRows');
			var checkboxs  = document.getElementsByName("clearId");//所有checkbox
			 var reqString ='['; 
			 var isChecked=false; 
			for(var i=0;i<rows.length;i++){//所有的 checkboxs 
				if(checkboxs[i].checked){
					...
					var clearDate = objBean.clearDate;
					if(clearDate==null||clearDate==''){
						$.messager.show('提示','报废清理日期不可为空', 'info');
						return false;
					}
					reqString +=jsonBean+',';
				}
			...
			$.post("${pageContext.request.contextPath}/pms/eqmScrapcel/addOrEditBean.do",pams,function(json){
				if (json.success) {
		...
		function deleteRow(){
			...
			$.post("${pageContext.request.contextPath}/pms/eqmScrapcel/deleteById.do",pams,function(json){
				if (json.success) {
					$.messager.show('提示','操作成功', 'info');
