...
<script type="text/javascript">
		...
		*/
		function queryTrouble() {
			dataGrid.datagrid({
				url : "${pageContext.request.contextPath}/pms/effect/queryDataGrid.do",
				queryParams :$("#searchForm").form("getData"),
				onLoadError : function(data) {
					$.messager.show('提示', "查询设备有效作业率", 'error');
				}
			});
			var f = $('#searchForm');
			$.post("${pageContext.request.contextPath}/pms/effect/queryGraph.do",f.form("getData"),function(obj){
				var xvalue=obj.xvalue;
				$('#container1').css('width',650+'px');
				var widthNum=parseInt((xvalue.length/9));//动态设置下面条形图宽度
				if(widthNum>=1){
					$('#container1').css('width',650*(widthNum+1)+'px');
				}
		...
		var loadView=function (xvalue,series) {
		 ...
		 //导出execl
		   function derive(){
						...
						var recordUrl ="${pageContext.request.contextPath}/pms/effect/deriveExcel.do?eqpName="+$("#searchForm").form("getData").eqpName+"&eqpType="+$("#searchForm").form("getData").eqpType+"&startTime="+$("#searchForm").form("getData").startTime+"&endTime="+$("#searchForm").form("getData").endTime;
...
<body class="easyui-layout" data-options="fit : true,border : false">
	<div id="toolbar" style="display: none;">
			...
			<div class="topTool">
				<fieldset >
					...
					<div >
						...
						<input id="date1" name="startTime" type="text" class="easyui-my97" datefmt="yyyy-MM-dd" style="width:130px"/>
					</div>	
					
					<div >
						<span class="label" style="width:10px">到</span>
						<input id="date2" name="endTime" type="text" class="easyui-my97" datefmt="yyyy-MM-dd" style="width:130px"/>					 
					</div>	
		...
		<div class="easyui-toolbar">
				...
				<a onclick="derive();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-standard-table-refresh'">导出Excel</a>
		</div>
	</div>
	<div data-options="region:'north',border:true,split:true" style="height: 300px;width:100%">
		<table id="dataGrid"></table>
	</div>
	<div data-options="region:'center',border:true,split:true,title:'有效作业率统计'" style="height: 200px;width:100%">
		<div id="container1" style="height: 300px;width:650px"></div>
