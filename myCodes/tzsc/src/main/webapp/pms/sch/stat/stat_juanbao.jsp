...
<script type="text/javascript">
		...
		$(function() {
			...
			outputGrid = $('#outputGrid').datagrid({
				...
				toolbar : '#outputToolbar',//toolbar属性让outputToolbar整个模块变成工具栏
			...
			/* $("#eqpType").combobox({
				onSelect:function(record){
					var eqptypeid=$("#eqpType").combobox('getValue');
					var url="${pageContext.request.contextPath}/pms/stat/geteqpment.do?eqptypeid="+eqptypeid;
					var param="";
					$.post(url,param,function(data){
						//alert(data);
						$("#eqp").html("");
						$("#eqp").html("<option value=''>全部</option>");
					   for(var i=0;i<data.eqpments.length;i++){
						   $(".easyui-combobox").append("<option value='"+data.eqpments[i].id+"'>"+data[i].eqpments[i].equipmentName+"</option>");
					   } 
					},"json");
				}
			}) */
...
<body class="easyui-layout" data-options="fit : true,border : false">
	<div data-options="region:'north',border:false,split:true,title:'产出数据'" style="height:320px;">
		<div id="outputToolbar"  style="display: none;width:100%;">
			<form id="outputForm" style="margin:4px 0px 0px 0px">
				<div class="topTool">
					...
					<div>
						...
						<select  id="eqpType" name="eqpType" class="eqptype" class="easyui-combobox" data-options="panelHeight:'110',width:100,editable:false">
					...
					<div>
						...
						<select id="equipment" name="equipment" class="easyui-combobox" data-options="panelHeight:200,width:100,editable:false"></select>
					</div>
					<div>
			...
			<div class="easyui-toolbar" >
				...
				<a onclick="sendDataToMES(2);" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-standard-package-go'" title="反馈本页本页数据到MES系统">反馈MES</a>
	...
	<div data-options="region:'center',border:false,split:true,title:'消耗数据'">
		<div class="easyui-toolbar" >
			...
			<a onclick="goToInputEditJsp()" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-standard-plugin-add'">编辑</a>
		</div>
		<table id="inputGrid"></table>
	</div>
	<div id="inputMenu" class="easyui-menu" style="width: 80px;">
