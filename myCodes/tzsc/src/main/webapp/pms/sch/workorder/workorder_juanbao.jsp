...
<script type="text/javascript">	
	...
	var id;  
	$(function() {
		/* $.loadComboboxData($("#type"),"EQPCATEGORY",true); */
		$.loadComboboxData($("#shift"),"SHIFT",true);
		$.loadComboboxData($("#team"),"TEAM",true);
		$.loadComboboxData($("#prodmat"),"MATPROD",true);
		var d=new Date();
	...
	*/	
	function getAllWorkOrder() {
		dataGrid.datagrid({
					...
					queryParams :$("#searchForm").form("getData"),//请求远程数据是发送的额外参数
					onLoadError : function(data) {//家在远程数据发生某些错误时触发
...
<form id="searchForm" style="margin:4px 0px 0px 0px">
			<div class="topTool">
				<fieldset >
					...
					<div >
						...
						<!--<select id="type" name="type" class="easyui-combobox" data-options="panelHeight:'auto',width:80">
							<option value="">全部</option>
							<option value="1">卷烟机</option>
							<option value="2">包装机</option>
							<option value="3">装封箱机</option>
							<option value="4">成型机</option>
							<option value="5">喂丝机</option>
							<option value="6">发射机</option>
							<option value="7">装盘机</option>
							<option value="8">卸盘机</option>
						</select>  -->
						
						<select id="type" name="type" class="easyui-combobox" data-options="panelHeight:150,width:'80',editable:'false'">
							<option value="">全部</option>
							<option value="1">卷烟机</option>
							<option value="2">包装机</option>
							<option value="3">装封箱机</option>
							<!-- <option value="4">成型机</option> -->
							<option value="5">发射机</option>
							<option value="6">装盘机</option>
							<option value="7">卸盘机</option>
						</select>
					...
					<div>
						...
						<select id="prodmat" name="mat" class="easyui-combobox" data-options="panelHeight:'150',width:'100',editable:'false'"></select>
					</div>
		...
		<div class="easyui-toolbar">
			...
			<c:if test="${not empty sessionInfo.resourcesMap['/pms/workorder/beatchDeleteWorkOrders.do']}">
				<a onclick="beatchDeleteWorkOrders();" href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-standard-cancel',plain:true">批量删除工单</a>
			</c:if>
		</div>
</div>
...
<div id="menu" class="easyui-menu" style="width: 80px; display: none;">
	...
	<c:if test="${not empty sessionInfo.resourcesMap['/pms/workorder/deleteWorkOrders.do']}">
