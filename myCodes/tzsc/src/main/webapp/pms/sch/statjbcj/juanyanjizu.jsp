...
<script type="text/javascript">
		...
		$(function() {
			...
			//初始化
			$.loadComboboxData($("#mdEquipmentSb"),"ALLROLLERS",true);//加载下拉框数据
			...
			passRateGrid = $('#passRateGrid').datagrid({
				...
				frozenColumns:[[//固定表头
						...
						{title:'设备编号',sortable : true, rowspan:3,field:'equipmentNameCode',width:60,align:'center'},//跨列
						
						{title:'设备名称',sortable : true, rowspan:3,field:'equipmentName',width:60,align:'center'},//跨列
						...
						{title:'产量(箱)', rowspan:3,field:'qty',width:60,align:'center'},//跨列
						//{title:'单位', rowspan:3,field:'mdUnitName',width:40,align:'center'},//跨列
						{title:'剔除(箱)',formatter: function(value,row,index){
							return Math.round(value*100)/100;
						}, rowspan:3,field:'badQty',width:60,align:'center'},//跨列
					
				]],
				columns : [  [ 
								
								{title:'辅料消耗',colspan:6},
						...
						{title : '运行时间(分钟)',rowspan:2,field : 'runTime',width :$(this).width() * 0.05,align:'center',sortable : true},
					...
					/* 	{title :'数据最后接收时间',rowspan:2,field : 'lastRecvTime',width :$(this).width() * 0.065,align:'center',sortable : true}, */
						...
						{title : '单耗(支/箱)',nowrap:false,rowspan:1,field : 'danhao1',align:'center',sortable : true},
						...
						{title : '消耗(KG)',rowspan:1,field : 'qty3',width :$(this).width() * 0.07,align:'center',sortable : true},
		...
		function getShapes() {
		    ...
		    if(date1!=null&date1!=""&&date2!=null&date2!=""){
		    	passRateGrid.datagrid({
					url : tempUrl+"/getAllJuanYanJiZu.do",
	   ...
	   //导出execl
	   function derive(){
	 	var recordUrl =tempUrl+"/excelDeriveJuanYanJZ.do?mdEquipmentId="+$("#passRateForm").form("getData").mdEquipmentId+"&stim="+$("#passRateForm").form("getData").stim+"&etim="+$("#passRateForm").form("getData").etim;
	 	var isPass=false;
	 	var queryExcel =tempUrl+"/queryExcelDeriveJuanYanJZ.do?mdEquipmentId="+$("#passRateForm").form("getData").mdEquipmentId+"&stim="+$("#passRateForm").form("getData").stim+"&etim="+$("#passRateForm").form("getData").etim;
